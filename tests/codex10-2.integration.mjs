...
test("normal simulation completes 1, 10, and 162 games", () => {
  ...
  for (const gameCount of [1, 10, 162]) {
    ...
    for (let index = 0; index < gameCount; index += 1) {
      ...
      assert.equal(state.isComplete, true, `${gameCount} games: index ${index}`);
      completed += 1;
    }
...
test("high-speed measurement completes 100, 1000, and 10000 games", async () => {
  const teams = createMlbAverageValidationTeams();
