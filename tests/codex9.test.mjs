...
test("measurement page is separate and cache-busted module entrypoints stay wired", async () => {
  ...
  assert.match(indexSource, /main\.js\?v=codex10-1/);
  assert.match(mainSource, /appBootstrap\.js\?v=codex10-1/);
  assert.match(bootstrapSource, /appRouter\.js\?v=codex10-1/);
  assert.match(bootstrapSource, /router\.js\?v=codex10-1/);
  assert.match(bootstrapSource, /rootStateFactory\.js\?v=codex10-1/);
  assert.match(routerSource, /appRouter\.js\?v=codex10-1/);
  assert.doesNotMatch(tuningSource, /measurementProgress|tuningMeasurementWorker/);
  assert.match(runnerSource, /new URL\(/);
  assert.match(runnerSource, /tuningMeasurementWorker\.js\?v=codex10-1/);
