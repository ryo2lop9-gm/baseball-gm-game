...
test("advanced aggregate groups and entity totals reconcile", async () => {
  ...
  assert.equal(summary.reportSchemaVersion, 8);
  assert.ok(pitches > 0);
...
test("schema v8 report carries diagnostics, definitions, and explicit limitations", async () => {
  const teams = createMlbAverageValidationTeams();
  const summary = await runMeasurementBatches({
    awayTeam: teams.away,
    homeTeam: teams.home,
  ...
  assert.equal(report.reportSchemaVersion, 8);
  for (const key of [
    "definitions",
    "modelLimitations",
    "gameDistribution",
  ...
  assert.equal(JSON.parse(json).reportSchemaVersion, 8);
});
